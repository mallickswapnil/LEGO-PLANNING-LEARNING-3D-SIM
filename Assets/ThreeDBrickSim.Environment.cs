...
public partial class ThreeDBrickSim
{
    ...
    public bool PickOrientAndPlaceBrick(string brickId, Vector3 orientationEuler, Vector3 targetLocation)
    {
        ...
        bool requiresPlaexSideSnap =
            ShouldRequirePlaexSideSnapConnection(
                brickTransform,
                targetLocation,
                targetRotation);
        ...
        Quaternion targetRotation)
    {
        ...
        return IsWithinRequiredPlaexSideSnapWindow(
            movingBrick,
            targetPosition,
            targetRotation);
    }
    ...
    private bool HasPlaexSideSnapJointConnection(Transform movingBrick)
    {
        ...
        for (int i = 0; i < joints.Length; i++)
        {
            ...
            if (!ArePlaexSideSnapBricksCompatible(movingBrick, connectedBrick))
            {
                continue;
            }
        ...
        Quaternion targetRotation)
    {
        ...
        try
        {
            ...
            foreach (Transform placedBrick in plannedWallBricks)
            {
                ...
                if (!HasCompatiblePlaexSideAlignmentAtCurrentPose(
                    movingBrick,
                    movingConnectors,
                    placedBrick))
                {
        ...
        bool requireExistingJointForAllowedInterlock = true)
    {
        ...
        if (!ArePlaexSideSnapBricksCompatible(movingBrick, otherBrick))
        {
            return false;
        }
        ...
        if (!HasCompatiblePlaexSideAlignmentAtCurrentPose(movingBrick, otherBrick))
        {
            return false;
        }
